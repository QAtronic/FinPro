...
export default function TemplatePreview({ params }: TemplatePreviewProps) {
  ...
  const getColorClasses = (colorScheme: string) => {
    const colorMap: { [key: string]: { primary: string, secondary: string, accent: string, text: string } } = {
      ...
      "Green & Brown": { primary: "bg-green-600", secondary: "bg-amber-600", accent: "bg-green-50", text: "text-green-600" },
      "Brown & Cream": { primary: "bg-amber-800", secondary: "bg-amber-50", accent: "bg-amber-50", text: "text-amber-800" },
      "Red & Gold": { primary: "bg-red-600", secondary: "bg-yellow-500", accent: "bg-red-50", text: "text-red-600" },
    }
      ...
      <section className="relative h-96 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center">
          <div className="text-center text-white">
            <h1 className="text-5xl font-bold mb-4">{template.bannerText || template.name}</h1>
            <p className="text-xl mb-8 max-w-2xl">{template.bannerSubtext || template.description}</p>
